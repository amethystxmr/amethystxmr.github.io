...
import { getWasmPthreadPoolSize } from "./wasmConcurrency";
...
export type LoadUiModule = () => Promise<{ default: React.ComponentType }>;
...
/**
 * Bootstraps the wallet runtime before mounting the UI: it selects the WASM
 * build variant, registers the isolation service worker when needed, initializes
 * the wallet module, then renders the UI module returned by `loadUiModule`.
 */
export function AppBootstrap({ loadUiModule }: { loadUiModule: LoadUiModule }) {
  ...
  loadUiModuleRef.current = loadUiModule;
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        await initModule(
          {
            ...
            pthreadPoolSize: getWasmPthreadPoolSize(),
          },
          (progress) => {
            if (cancelled) {
              return;
        ...
        const { default: LoadedUi } = await loadUiModuleRef.current();
        if (cancelled) {
          return;
        }
